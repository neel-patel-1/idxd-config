...
struct acctest_context *dsa, *iaa_scan, *iaa_select;
...
static int iaa_init_select_task(struct task *tsk, int tflags,
...
static int setup_operations(int buf_size, int num_desc) {
	struct task_node *dsa_tsk_node, *iaa_scan_tsk_node, *iaa_select_tsk_node;
	...
	rc = acctest_alloc_multiple_tasks(iaa_scan, num_desc);
	if (rc != ACCTEST_STATUS_OK)
		return rc;
    rc = acctest_alloc_multiple_tasks(iaa_select, num_desc);
	if (rc != ACCTEST_STATUS_OK)
		return rc;
	...
	iaa_scan_tsk_node = iaa_scan->multi_task_node;
    iaa_select_tsk_node = iaa_select->multi_task_node;
	...
	while (iaa_scan_tsk_node) {
        ...
        if (rc != ACCTEST_STATUS_OK)
            return rc;
        rc = iaa_init_select_task(iaa_select_tsk_node->tsk, tflags, IAX_OPCODE_SELECT, buf_size);
		if (rc != ACCTEST_STATUS_OK)
			return rc;
        rc = memcpy_init(dsa_tsk_node->tsk, tflags, DSA_OPCODE_MEMMOVE, buf_size);
		if (rc != ACCTEST_STATUS_OK)
			return rc;
		iaa_scan_tsk_node = iaa_scan_tsk_node->next;
        iaa_select_tsk_node = iaa_select_tsk_node->next;
...
int main(int argc, char *argv[])
{
	...
	// iaa scan setup
	iaa_scan = acctest_init(tflags);
	iaa_scan->dev_type = ACCFG_DEVICE_IAX;

	if (!iaa_scan)
		return -ENOMEM;

	rc = acctest_alloc(iaa_scan, wq_type, iaa_dev_id, iaa_scan_wq_id);
	if (rc < 0)
		return -ENOMEM;

    // iaa select setup
	iaa_select = acctest_init(tflags);
	iaa_select->dev_type = ACCFG_DEVICE_IAX;

	if (!iaa_select)
		return -ENOMEM;

	rc = acctest_alloc(iaa_select, wq_type, iaa_dev_id, iaa_select_wq_id);
	if (rc < 0)
		return -ENOMEM;
